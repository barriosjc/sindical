        ...
        return [
            ...
            'cuil' => [
                function ($attribute, $value, $fail) use ($rq) {
                    if (!($rq->input($attribute) == null)) {
                        if (!$rq->id) {
                            $afiliado = afiliado::where("cuil", $rq->cuil)->first();
                            if (!empty($afiliado)) {
                                $fail("Otro afiliado titular ya esta dado de alta con el mismo nro de CUIL");
                            }
                        }
                        $gru_fam = grupo_familiar::where("cuil", $value)->first();
                        if ($gru_fam) {
                            if (empty($gru_fam->fecha_egreso_sind)) {
                                $fail("Error al dar de alta, Ya existe un Familiar con este CUIL, primero de de baja el familiar y luego de alta el titular.");
                            }
                        }
                    }
                }
            ...
            'nro_afil_sindical' => ['required',
                function ($attribute, $value, $fail) use ($rq) {
                    if (!($rq->input($attribute) == null)) {
                        if (!$rq->id) {
                            $afiliado = afiliado::where("nro_afil_sindical", $rq->nro_afil_sindical)->first();
                            if (!empty($afiliado)) {
                                $fail("Otro afiliado titular ya esta dado de alta con el mismo nro de Afiliado Sindical");
                            }
                        }
                    }
                }
